import React from 'react'
const Warn = () => (
	<>
		<p>
			Ninguém está autorizado a utilizar o logo e nome da Associação de Diabetes
			Juvenil - ADJ, principalmente para fins comerciais.
		</p>
		<p>
			O uso indevido e sem autorização acarretará a tomada das medidas legais
			cabíveis.
		</p>

		<p>
			É vedada a publicação do conteúdo deste site, no todo ou em parte, sem
			prévia autorização, por escrito, da ADJ Diabetes Brasil.
		</p>
		<p>Solicite autorização pelo e-mail: </p>
		<p>
			<span>
				<a href="mailto:revistaadj@adj.org.br" className="footer-link">
					revistaadj@adj.org.br
				</a>
			</span>
		</p>
	</>
)
export default Warn
