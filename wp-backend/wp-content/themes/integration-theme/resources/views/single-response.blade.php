          ...
          <div class="row">
            @notempty($response)
              <div class="col-12 mb-2">
                @include('partials.posts.response.card', ['data' => $response, 'class_title' => 'text-primary'])
              </div>
            @endnotempty
            ...
            @notempty($endorsements)
              @foreach($endorsements as $data)
                <div class="col-12 mb-2">
                  @include('partials.posts.response.card', ['data' => $data, 'class_title' => 'text-primary'])
                </div>
              @endforeach
            @endnotempty
          </div>
            ...
            <div class="row">
              @foreach($financial as $data)
                <div class="col-12 col-md-6 mb-3">
                  @include('partials.posts.response.card', ['data' => $data])
                </div>
              @endforeach
            </div>
          </section>
