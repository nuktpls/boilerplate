...
class PostResponse {
    ...
    public function __construct() {
        add_action('add_meta_boxes_response', [$this, 'addMetaBoxes']);
        ...
        $this->createResponseDownloadSettings();
    }
    ...
    function createDownloadsSettings(string $key = 'downloads', string $title = 'Documentos'): void {
        register_extended_field_group([
            ...
            'style'    => 'default',
            'location' => [
                Location::if('post_type', 'response')
            ],
        ]);
    }

    function createResponseDownloadSettings(): void {
        register_extended_field_group([
            'key'      => 'download_response',
            'title'    => 'Resposta',
            'fields'   => $this->createResponseDownloadFields(),
            'style'    => 'default',
            'position' => 'side',
            'location' => [
                Location::if('post_type', 'response')
    ...
    function createResponseDownloadFields(): array {
        return [
            DatePicker::make('Data de publicação', 'response_date')
                ->displayFormat('d/m/Y'),
            File::make('Arquivo', 'response_file')
                ->returnFormat('url'),
        ];
    }
